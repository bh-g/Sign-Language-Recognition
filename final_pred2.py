...
class Application:
    ...
    def __init__(self):
        ...
        # self.speak_engine.setProperty('voice',"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\MSTTS_V110_hiIN_HemantM")
        ...
        self.language_var.set("Language")  # default value
        self.speak_button = tk.Menubutton(self.root, textvariable=self.language_var)
        self.speak_button.config(font=("Courier", 20), wraplength=150,relief=tk.RAISED, bd=2, padx=10, pady=10)
        self.speak_button.place(x=1309, y=630)

        self.menu = tk.Menu(self.speak_button, tearoff=0)
        self.menu.add_command(label="English", command=lambda: self.set_language("English"))
        self.menu.add_command(label="Hindi", command=lambda: self.set_language("Hindi"))
        self.menu.add_command(label="Tamil", command=lambda: self.set_language("Tamil"))
        self.speak_button.config(menu=self.menu)

        # Bind left-click to execute the speak action
        self.speak_button.bind("<Button-1>", self.speak_fun)
    ...
    def speak_hindi(self):
        voices=self.speak_engine.getProperty("voices")
        self.speak_engine.setProperty("voice",voices[2].id)
        text_eng = self.str
        translated = translator.translate(text_eng, src='en', dest='hi')
        text_hindi = translated.text
        print("Translated:", text_hindi)
        self.speak_engine.say(text_hindi)
        self.speak_engine.runAndWait()
    ...
    def speak_tamil(self):
        ...
        self.speak_engine.setProperty("voice",voices[1].id)
        text_eng = self.str
        translated = translator.translate(text_eng, src='en', dest='ta')
        text_hindi = translated.text
        print("Translated:", text_hindi)
        self.speak_engine.say(text_hindi)
        self.speak_engine.runAndWait()
